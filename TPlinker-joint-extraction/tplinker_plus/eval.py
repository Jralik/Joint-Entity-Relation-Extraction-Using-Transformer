...
best_model_path = os.path.join(model_state_dir, "best_model.pt")
run_id2model_state_paths = {}
if os.path.exists(best_model_path):
    run_id2model_state_paths["default"] = [best_model_path]
else:
...
if config["score"]:
    ...
    for file_path, pred_samples in res_dict.items():
        file_name = re.search("(.*?)_res_best\.json", file_path.split("/")[-1]).group(1)
