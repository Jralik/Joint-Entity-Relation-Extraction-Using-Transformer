...
if config["encoder"] == "BERT":
    ...
    get_tok2char_span_map = lambda text: tokenizer(text, return_offsets_mapping = True, add_special_tokens = False)["offset_mapping"]
...
def train_n_valid(train_dataloader, dev_dataloader, optimizer, scheduler, num_epoch):  
    ...
    patience = hyper_parameters.get("early_stopping_patience", 10)
    patience_counter = 0
    ...
    for ep in range(num_epoch):
        ...
        if valid_f1 >= max_f1: 
            max_f1 = valid_f1
            patience_counter = 0 # Reset counter nếu model cải thiện
            if valid_f1 > config["f1_2_save"]: # save the best model
                best_model_path = os.path.join(model_state_dict_dir, "best_model.pt")
                torch.save(rel_extractor.state_dict(), best_model_path)
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            print(f"Early stopping triggered! Model hasn't improved for {patience} epochs.")
            break
